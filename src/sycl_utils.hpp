...
//
// Usage:
//   sycl::queue q = plsf::make_queue(plsf::DevicePreference::GPU);
//   plsf::print_device_info(q);
  ...
  try
  ...
  catch (const sycl::exception &)
    {
      if (pref == DevicePreference::GPU)
        {
          std::cerr
              << "[pLSF] Warning: no GPU found, falling back to CPU device.\n";
          dev = sycl::device{ sycl::cpu_selector_v };
        }
      else
        {
...
inline void print_device_info (
    const sycl::queue &q, std::ostream &os = std::cout)
{
